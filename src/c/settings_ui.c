...
static char s_course_date_buffer[16];
static char s_service_date_buffer[16];
...
static void format_date(DateParts date, char *buffer, size_t buffer_size) {
...
static void refresh_menu(void) {
  ...
  snprintf(s_pagoda_buffer, sizeof(s_pagoda_buffer), "%d", s_settings->pagoda_cell);
  snprintf(s_dining_buffer, sizeof(s_dining_buffer), "%d", s_settings->dining_hall);
  snprintf(s_cushion_buffer, sizeof(s_cushion_buffer), "%d", s_settings->cushion);
...
static void editor_adjust(int direction) {
  if (s_editor_type == EDITOR_COURSE_DATE || s_editor_type == EDITOR_SERVICE_DATE) {
    DateParts *target = s_editor_type == EDITOR_COURSE_DATE ? &s_settings->course_start
                                                            : &s_settings->service_start;
    time_t base = settings_date_to_time(*target);
    base += direction * 86400;
    settings_date_from_time(target, base);
  } else if (s_editor_type == EDITOR_PAGODA) {
    s_settings->pagoda_cell = s_settings->pagoda_cell + direction;
  } else if (s_editor_type == EDITOR_DINING) {
    s_settings->dining_hall = s_settings->dining_hall + direction;
  } else if (s_editor_type == EDITOR_CUSHION) {
    s_settings->cushion = s_settings->cushion + direction;
  }
