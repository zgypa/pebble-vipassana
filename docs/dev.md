...
## Versioning (Minimal Bump Steps)

The watchface version is stored in `package.json` under `"version"`. The build generates `build/appinfo.json` from it.

To bump a version:

1. Update the version in `package.json`.
2. Update the changelog in `README.md` to match the new version.
